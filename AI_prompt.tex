...
我从神经科学的和模型出发得到了一个具有生物合理性的向后传播误差信号的等效过程以及依据这个误差信号更新连接权重的具有生物合理性的学习规则。现在我想要检验这个传播误差的过程和学习规则在深度学习中的效果。但是我不确定这样的规则直接验证是否不可行，给出一些调整我的模型以便于可以和传统的深度学习算法匹配的建议
...
\]
这里的$\zeta$是一个截断函数，用于将值局限在$\left(-\frac{1}{2},\frac{1}{2}\right)$内。然后更新权重的法则也变成了
\[
...
我从神经科学的和模型出发得到了一个具有生物合理性的向后传播误差信号的等效过程以及依据这个误差信号更新连接权重的具有生物合理性的学习规则。现在我想要检验这个传播误差的过程和学习规则在深度学习中的效果。根据下面的具体细节构建一个验证这个新的学习规则的Pytorch代码。
...
\Delta W_{\left(l\right)}=-\eta f\left(\varepsilon_{\left(l\right)},y_{\left(l\right)}\right)\odot y^{T}_{\left(l-1\right)}
\]
其中$f\left(\varepsilon_{\left(l\right)},y_{\left(l\right)}\right)$是一个多元函数，其输出结果和$\varepsilon_{\left(l\right)}$的大小一样。例如它可以是$f\left(\varepsilon_{\left(l\right)},y_{\left(l\right)}\right)=\varepsilon_{\left(l\right)}\odot y_{\left(l\right)}$，那么它除了截断函数$\zeta$就和原来的更新规则没有区别了。在我的模型中$f$有多种情况，我会按照自己的需求测试。
\end{document}
